...
class AutoEncoder(nn.Module):
    ...
    def forward(self, x: torch.Tensor, goal: torch.Tensor):
        x = x.expand(-1, 3, -1, -1)
        print(f"Input x after expand: {x.shape}")
        x = self.encoder(x)
        print(f"Output from encoder: {x.shape}")
        x, c = self.decoder(x, goal)
        print(f"Output from decoder: {x.shape}, {c.shape}")
        return x, c
...
class DualAutoEncoder(nn.Module):
    def __init__(
        ...
        print(f"Mask2Former Config Enabled: {m2f_cfg}") #debug print
        ...
        if train_cfg.rgb and train_cfg.pre_train_sem and PRE_TRAIN_POSSIBLE:
            ...
            print("Using Mask2Former for semantic encoding.") #debug
        else:
            self.encoder_sem = PlannerNet(layers=[2, 2, 2, 2])
            print("Using default PlannerNet for semantic encoding.")
    ...
    def forward(self, x_depth: torch.Tensor, x_sem: torch.Tensor, goal: torch.Tensor):
        #print("Calling forward pass in the DualAutoEncoder model...")
        # encode depth
        print(f"x_depth initial shape: {x_depth.shape}", flush=True)
        print(f"Input x_depth shape: {x_depth.shape}")
        x_depth = x_depth.expand(-1, 3, -1, -1)
        print(f"Expanded x_depth shape: {x_depth.shape}")
        x_depth = self.encoder_depth(x_depth)
        print(f"Encoded x_depth shape: {x_depth.shape}")

        # encode sem
        x_sem = self.encoder_sem(x_sem)
        print(f"Encoded x_sem shape: {x_sem.shape}")
        # concat
        ...
        # decode
        print(f"Shape before decoder: {x.shape}")
        x, c = self.decoder(x, goal)
        return x, c
